...
export default function SessionScreen() {
  ...
  useEffect(() => { setOrderIds(null); }, [idsKey]);

  const orderedTracks = useMemo(() => {
    if (!orderIds) return tracks;
    const map = new Map(tracks.map((t) => [t.id, t]));
    const arr = orderIds.map((tid) => map.get(tid)).filter(Boolean) as Track[];
    tracks.forEach((t) => { if (!orderIds.includes(t.id)) arr.push(t); });
    return arr;
  }, [orderIds, tracks]);
  ...
  // metronome follows playback
  useEffect(() => {
    if (player.isPlaying && metronomeOn && !recording) metro.start(data?.bpm || 90);
    else if (!recording) metro.stop();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [player.isPlaying, metronomeOn]);
  ...
  const reorder = useMutation({
    mutationFn: (trackIds: string[]) => apiFetch(`/sessions/${id}/reorder`, { method: "POST", body: JSON.stringify({ track_ids: trackIds }) }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["session", id] }),
  });
  ...
  useEffect(() => () => { if (barTimer.current) clearInterval(barTimer.current); metro.stop(); }, []);
  ...
  const beginRecordingNow = async () => {
    try {
      ...
      recorder.record();
      setRecording(true);
      if (metronomeOn) metro.start(data?.bpm || 90);
  ...
  const startRecording = async () => {
    ...
    if (data?.count_in) {
      setCountdown(4);
      await metro.countIn(data?.bpm || 90, 4, (r) => setCountdown(r > 0 ? r : null));
      setCountdown(null);
    }
  ...
  const renderLane = ({ item: t, drag, isActive }: { item: Track; drag: () => void; isActive: boolean }) => {
    const col = trackColors[t.color % trackColors.length];
    return (
      <ScaleDecorator>
        <View style={[styles.lane, isActive && { borderColor: col, ...glow(col, 14, 0.6) }]} testID={`track-lane-${t.id}`}>
          <View style={styles.laneControls}>
            <View style={styles.laneNameRow}>
              <Pressable onLongPress={drag} delayLongPress={120} hitSlop={8} testID={`drag-${t.id}`} style={styles.dragHandle}>
                <DotsSixVertical size={16} color={colors.muted} weight="bold" />
              </Pressable>
            ...
            <View style={styles.msRow}>
              <Pressable onPress={() => toggleMute(t)} testID={`mute-${t.id}`} style={[styles.msBtn, t.muted && { backgroundColor: colors.warning, borderColor: colors.warning }]}>
                <Text style={[styles.msText, t.muted && { color: colors.onWarning }]}>M</Text>
              </Pressable>
              <Pressable onPress={() => toggleSolo(t)} testID={`solo-${t.id}`} style={[styles.msBtn, t.solo && { backgroundColor: colors.brandSecondary, borderColor: colors.brandSecondary }]}>
    ...
    <View style={styles.root}>
      ...
      <View style={{ flex: 1 }}>
        <DraggableFlatList
          ...
          onDragEnd={({ data: nd }) => { setOrderIds(nd.map((t) => t.id)); reorder.mutate(nd.map((t) => t.id)); }}
      ...
      {/* Count-in overlay */}
      {countdown !== null && (
        <View style={styles.countIn} testID="count-in-overlay" pointerEvents="none">
          <Text style={styles.countInLabel}>COUNT-IN</Text>
          <Text style={styles.countInNum}>{countdown}</Text>
        </View>
      )}
      ...
      <View style={[styles.transport, { paddingBottom: insets.bottom + 8 }]}>
        <Pressable
          ...
          style={[styles.transportBtn, metronomeOn && { borderColor: colors.brandSecondary, ...glow(colors.brandSecondary, 10, 0.5) }]}
        >
        ...
        <Pressable onPress={player.toggle} disabled={tracks.length === 0} testID="transport-play" style={[styles.playBtn, { opacity: tracks.length === 0 ? 0.4 : 1 }]}>
...
const useStyles = makeStyles((colors) => ({
  ...
  title: { fontFamily: fonts.displayBold, fontSize: 16, color: colors.onSurface, letterSpacing: 0.5, maxWidth: 210 },
  timecode: { fontFamily: fonts.numeric, fontSize: 15, color: colors.brandSecondary, letterSpacing: 1 },
  ...
  lane: {
    ...
    padding: 10, gap: 10, minHeight: 108, marginBottom: 10,
  },
  laneControls: { width: 150, gap: 6 },
  laneNameRow: { flexDirection: "row", alignItems: "center", gap: 5 },
  dragHandle: { width: 22, height: 26, alignItems: "center", justifyContent: "center" },
  colorDot: { width: 10, height: 10, borderRadius: 5 },
  laneName: { fontFamily: fonts.textMedium, fontSize: 13, color: colors.onSurface, flex: 1 },
  ...
  msText: { fontFamily: fonts.displayBold, fontSize: 12, color: colors.onSurface },
  ...
  countIn: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(13,13,18,0.82)", zIndex: 30 },
  countInLabel: { fontFamily: fonts.displayBold, fontSize: 16, color: colors.brandSecondary, letterSpacing: 4 },
  countInNum: { fontFamily: fonts.displayBlack, fontSize: 120, color: colors.onSurface, textShadowColor: colors.brandPrimary, textShadowRadius: 24 },
  recOverlay: {
  ...
  recTimer: { fontFamily: fonts.numeric, fontSize: 32, color: colors.onSurface, letterSpacing: 1 },
  ...
  tBpm: { fontFamily: fonts.numeric, fontSize: 13 },
